  ...
  async run(job,indices) {
    try {
      ...
      for (const index of indices) {
        ...
        out.generatedFilename=`${job.execution==='MOCK' && job.mode!=='CROP_ZOOM' ? 'mock_' : ''}angle_${String(index+1).padStart(2,'0')}_${job.mode==='GENERATIVE_ANGLE' ? out.angle.toLowerCase() : 'source'}_${out.framing.toLowerCase()}${out.history.length ? `_v${out.history.length+1}` : ''}.png`;
        await this.persist(job);
        try {
          let buffer;
          if (job.mode==='CROP_ZOOM') {
            ...
            const parameters={model:job.config.model,size:job.config.size,quality:job.config.quality,n:1,output_format:'png',referenceCount:buffers.length,inputImageCount:inputs.length,mask:false,promptGuidedOutpaint:!!mask};
            ...
            if (job.execution==='MOCK') buffer=await mockImage(buffers[out.sourceIndex],out,job.mode,job.config.size);
              ...
              const result=await editImage({...this.config,...job.config},inputs,out.prompt,mask,this.transport,async progress=>{out.providerProgress=progress.phase;if(progress.requestId)out.requestId=progress.requestId;await this.persist(job);});
