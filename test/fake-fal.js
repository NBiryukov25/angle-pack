export function fakeFal(image,{onSubmit=()=>{},failAt=0,result={images:[{url:'https://fal.media/result.png'}],seed:123}}={}) {
  ...
  return {uploads,get count(){return count;},sleep:async()=>{},upload:async file=>{uploads.push(file);return `https://fal.media/ref${uploads.length}.png`;},fetch:async(url,options={})=>{
    ...
    if(url.startsWith('https://queue.fal.run/'))return Response.json(result);
