...
function setBusy(busy){state.busy=busy;$('upload').disabled=busy;$('model').disabled=busy;$('compare').disabled=busy||!MODES[state.mode].requiresModels;$('execution').disabled=busy;$('sessions').disabled=busy;$('notes').disabled=busy;$('preserve-all').disabled=busy;document.querySelectorAll('[data-mode],#preservation select').forEach(n=>n.disabled=busy);renderCounts();renderOutputs();renderReferences();$('auto').disabled=busy||state.mode!=='GENERATIVE_ANGLE';}
...
function renderGallery() {
  ...
  job.outputs.forEach((out,i)=>{
    ...
    if(out.status==='complete') {const link=el('a',{href:fileUrl(job,out.generatedFilename),target:'_blank',rel:'noopener'});link.append(el('img',{src:fileUrl(job,out.generatedFilename),alt:`Output ${i+1}: ${MODES[job.mode]?.usesAngle?ANGLES[out.angle]:'source view'}, ${FRAMINGS[out.framing]}`}));card.append(link);}
    ...
    for(const title of ['REGENERATE',...(MODES[job.mode]?.usesAngle?['CHANGE ANGLE']:[]),'CHANGE ZOOM/FRAMING']){const button=el('button',{},title);button.disabled=state.busy;button.onclick=()=>openEdit(i,title);actions.append(button);}
...
async function generate() {
  ...
  try {
    ...
    form.append('spec',JSON.stringify({mode:state.mode,execution:currentExecution(),outputs,preservation:state.preservation,referenceAngles:state.refs.map(r=>r.angle),notes:$('notes').value,...(state.model?{model:state.model}:{})}));
...
function openEdit(index,action) {
  ...
  const refresh=()=>{const refs=state.job.references.map((r,i)=>({url:`/api/sessions/${state.job.id}/reference/${i}`,name:r.originalName}));$('edit-controls').replaceChildren(outputControls(state.edit.output,index,state.job.mode,refs,refresh,{showModel:true}));};refresh();
