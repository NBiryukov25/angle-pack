...
async function api(url,options={}) {
  const headers={...options.headers};if(options.method){headers['X-Angle-Pack-Token']=state.config.token;}
...
function setMode(mode) {
  state.mode=mode;document.querySelectorAll('[data-mode]').forEach(b=>{b.classList.toggle('selected',b.dataset.mode===mode);b.setAttribute('aria-pressed',String(b.dataset.mode===mode));});
...
function setBusy(busy){state.busy=busy;$('upload').disabled=busy;$('execution').disabled=busy;$('sessions').disabled=busy;$('notes').disabled=busy;$('preserve-all').disabled=busy;document.querySelectorAll('[data-mode],#preservation select').forEach(n=>n.disabled=busy);renderCounts();renderOutputs();renderReferences();$('auto').disabled=busy||state.mode!=='GENERATIVE_ANGLE';}
...
function renderGallery() {
  ...
  $('manifest').hidden=false;$('manifest').href=fileUrl(job,'manifest.json')+'?download=1';
  ...
  job.outputs.forEach((out,i)=>{
    ...
    if(out.status==='complete') {const link=el('a',{href:fileUrl(job,out.generatedFilename),target:'_blank',rel:'noopener'});link.append(el('img',{src:fileUrl(job,out.generatedFilename),alt:`Output ${i+1}: ${job.mode==='GENERATIVE_ANGLE'?ANGLES[out.angle]:'source view'}, ${FRAMINGS[out.framing]}`}));card.append(link);}
    else card.append(el('div',{class:'result-placeholder'},out.status==='running'?'Creating this image…':out.status));
    ...
    for(const title of ['REGENERATE',...(job.mode==='GENERATIVE_ANGLE'?['CHANGE ANGLE']:[]),'CHANGE ZOOM/FRAMING']){const button=el('button',{},title);button.disabled=state.busy;button.onclick=()=>openEdit(i,title);actions.append(button);}
    if(out.status==='complete'){const link=el('a',{href:fileUrl(job,out.generatedFilename)+'?download=1'},'Download PNG');body.append(link);}
    body.append(actions);
    const completeVersions=out.history.filter(o=>o.status==='complete');
    if(completeVersions.length){const versions=el('details');versions.append(el('summary',{},'Previous versions'));for(const prior of completeVersions){const p=el('p'),a=el('a',{href:fileUrl(job,prior.generatedFilename),target:'_blank',rel:'noopener'},prior.generatedFilename);p.append(a);versions.append(p);}body.append(versions);}
...
async function followJob(job) {
  ...
  if(state.busy)pollTimer=setTimeout(async()=>{try{await followJob(await api(`/api/sessions/${job.id}`));}catch(error){message(`Progress connection lost: ${error.message} The server may still be generating. Reconnecting…`);pollTimer=setTimeout(()=>followJob(job),2500);}},1000);
...
async function generate() {
  ...
  setBusy(true);
  try {
    const form=new FormData();for(const ref of state.refs){const file=ref.file||await fetch(ref.url).then(r=>{if(!r.ok)throw new Error('Could not load saved reference');return r.blob();});form.append('references',file,ref.name);}
    const outputs=state.outputs.map(o=>({...o,...(state.mode!=='GENERATIVE_ANGLE'?{angle:'FRONT',custom:''}:{})}));
    form.append('spec',JSON.stringify({mode:state.mode,execution:currentExecution(),outputs,preservation:state.preservation,referenceAngles:state.refs.map(r=>r.angle),notes:$('notes').value}));
...
function openEdit(index,action) {
  ...
  const refresh=()=>{const refs=state.job.references.map((r,i)=>({url:`/api/sessions/${state.job.id}/reference/${i}`,name:r.originalName}));$('edit-controls').replaceChildren(outputControls(state.edit.output,index,state.job.mode,refs,refresh));};refresh();
...
async function loadSession(id) {
  ...
  state.refs=job.references.map((r,i)=>({name:r.originalName,url:`/api/sessions/${id}/reference/${i}`,angle:r.angle}));
...
$('logout').onclick=async()=>{try{await api('/api/auth/logout',{method:'POST'});location.assign('/login');}catch(error){message(error.message);}};
async function init(){state.config=await api('/api/config');$('logout').hidden=!state.config.authEnabled;$('environment').textContent=state.config.mockOnly?'MOCK / DEVELOPMENT':state.config.hosted?'PRIVATE WORKSPACE':'LOCAL WORKSPACE';$('execution').querySelector('[value="LIVE"]').disabled=!state.config.liveAvailable;renderPreservation();renderCounts();renderReferences();setMode(state.mode);await refreshSessions();}
init().catch(error=>message(`Could not connect to ANGLE PACK: ${error.message}`));
